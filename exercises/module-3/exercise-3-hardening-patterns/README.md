...
แบบฝึกหัดนี้จะพาเราปรับแต่ง Agent ให้ตอบอย่างมีขอบเขต ชัดเจน และคาดเดาได้มากขึ้น โดยต่อยอดจาก Financial Report Assistant ที่สร้างใน Module 2 และปรับ reliability pattern มาแล้วใน Module 3
...
2. เรามาลองโหวตว่า Agent ตอบคำถามแบบไหน

   - ✅ Agent รู้ข้อมูลนี้จริง
   - ⚠️ Agent กำลังเดาข้อมูล

3. ให้ทดลองออกไอเดียคำตอบใหม่เป็น 1–2 ประโยค โดย Agent ไม่เดาข้อเท็จจริงที่ยังไม่มีข้อมูลยืนยัน

   <details>
   <summary>ตัวอย่างคำตอบที่ปลอดภัย</summary>
...
4. ให้ทดลองออกไอเดียคำตอบใหม่​ ในกรณีที่ Agent **มีข้อมูลยืนยันแล้ว** (ตอบพร้อม citation/reference)

   <details>
   <summary>ตัวอย่างคำตอบ</summary>
...
6. เปิดแต่ละ challenge ต่อไปนี้ทีละข้อ โดยเขียนคำตอบที่ปลอดภัยกว่าใน chat
   ...
   > **💡 Note:**  ถ้าไม่มีข้อมูลยืนยัน Agent ไม่ควรเดา ควรบอกข้อจำกัดหรือขอข้อมูลที่จำเป็นเพิ่ม
...
Practice นี้ช่วยให้ Agent รู้ว่าเรื่องใดอยู่ในขอบเขตของตนเอง และควร redirect ผู้ใช้อย่างไรเมื่อคำขออยู่นอกขอบเขตการทำงานของ Agent


> **💡 Note:** คำขอที่อยู่นอก scope ควร redirect ไปยังผู้รับผิดชอบในขอบเขตงานที่เหมาะสม ส่วนคำขอที่อยู่ใน scope แต่มีความเสี่ยงหรือเกี่ยวกับการอนุมัติ ให้ใช้แนวคิด Escalate จาก Exercise 2
...
2. มาโหวตกันว่า Agent ควรตอบคำขอนี้หรือไม่

   - ✅ อยู่ใน scope
   - 🚫 อยู่นอก scope

3. ตัวอย่างคำตอบที่เหมาะสมกว่าเมื่อคำขออยู่นอก scope
...
4. ลองเปิดดู challenge ต่อไปนี้ทีละข้อ และเขียนคำตอบ redirect ที่ปลอดภัยกว่าใน chat
...
6. แชร์คำตอบ redirect ที่คิดว่าดีที่สุดใน Teams chat พร้อมอธิบาย 1 บรรทัดว่า Agent ได้รักษาขอบเขตการทำงานของมันอย่างไร
...
1. ลองพิจารณาคำขอต่อไปนี้
   ...
   ```text
   User: ช่วยเตรียม executive summary จากรายงานนี้ แล้วส่งให้ Finance Director 

   Agent: ได้ครับ ผมเตรียม executive summary และส่งให้ Finance Director เรียบร้อยแล้ว
   ```

2. มาลองโหวตว่า Agent ควรทำแบบใด

   - ✅ Can do now
   - ⚠️ Need confirmation

3. แนวทางที่เหมาะสมกว่าเมื่อคำขอมี action ที่ต้อง**ควร หรือต้อง**ยืนยันก่อนส่ง เผยแพร่ หรือเปลี่ยนแปลงข้อมูล
...
4. ลองเลือกดู chalenge ต่อไปนี้ และลองเขียนข้อความขอยืนยันที่ปลอดภัยกว่าใน chat
...
5. แชร์คำตอบที่คิดว่าดีที่สุดใน Teams chat พร้อมอธิบาย 1 บรรทัดว่า Agent ได้ขอการยืนยันก่อนทำ action ที่มีผลต่อผู้อื่นอย่างไร

---

## Practice 4: Make It Chat-Friendly

Practice นี้ช่วยให้ Agent เปลี่ยนข้อความที่ยาวหรืออ่านยาก ให้สั้น ชัดเจน และใช้งานได้ดี โดยไม่เปลี่ยนความหมายเดิม


1. ลองพิจารณาข้อความนี้
...
2. มาโหวตว่าข้อความนี้เหมาะกับการอ่านระหว่างการใช้งาน agent หรือไม่

   - ✅ อ่านง่ายแล้ว
   - ⚠️ ควรแก้ไข

3. แนวทางที่เหมาะสมกว่าเมื่อข้อความยาวหรืออ่านยากเกินไปสำหรับการใช้งานใน chat
...
4. ลองเลือก challenge ต่อไปนี้ และลอง rewrite คำตอบให้อ่านง่ายขึ้นโดยไม่เปลี่ยนความหมายเดิม
...
5. แชร์คำตอบที่คิดว่าดีที่สุดใน Teams chat พร้อมอธิบาย 1 บรรทัดว่า Agent ได้ทำให้ข้อความอ่านง่ายขึ้นอย่างไร โดยไม่เปลี่ยนความหมายเดิม
---
